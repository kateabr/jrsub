...
class YarxiTests(unittest.TestCase):
    ...
    def test_simple_translation(self):
        self.assertEqual(
            self.yd.lookup_translations_only("亜塩素酸塩", "あえんそさんえん"), ["《хим.》 хлорит"]
        )

    def test_multiple_wordforms_and_readings(self):
        self.assertEqual(
            self.yd.lookup_translations_only("青図面", "あおずめん"),
            self.yd.lookup_translations_only("青図", "あおず"),
        )

        # not really correct, but must work nonetheless
        self.assertEqual(
            self.yd.lookup_translations_only("青図", "あおずめん"),
            self.yd.lookup_translations_only("青図面", "あおず"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("赤鳥赤腹", "あかこっこ"),
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("匕首", "あいくち"),
            self.yd.lookup_translations_only("匕首", "ひしゅ"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("エートス", "ええとす"),
            self.yd.lookup_translations_only("エトス", "えとす"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("クーリングオフ期間", "くうりんぐおふきかん"),
            self.yd.lookup_translations_only("クーリングオフ", "くうりんぐおふ"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("アイスクリームサンデー", "あいすくりいむさんでえ"),
            self.yd.lookup_translations_only("クリームサンデー", "くりいむさんでえ"),
        )

    def test_retrieving_references(self):
        self.assertEqual(
            self.yd.lookup_translations_only("蚶", "あかがい"),
            self.yd.lookup_translations_only("赤貝", "あかがい"),
        )
        self.assertCountEqual(
            self.yd.lookup_translations_only("合口", "あいくち"),
            [
                "общий язык, возможность приятно побеседовать; приятный " "собеседник",
                "кинжал без гарды",
            ],
        )
    ...
    def test_translation(self):
        # complete synonyms
        self.assertEqual(
            self.yd.lookup_translations_only("漢民族", "かんみんぞく"),
            self.yd.lookup_translations_only("漢族", "かんぞく"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("亜ぐ", "つぐ"),
            self.yd.lookup_translations_only("次ぐ", "つぐ"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("慣用句", "かんようく"),
            self.yd.lookup_translations_only("慣用語句", "かんようごく"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("懽楽", "かんらく"),
            self.yd.lookup_translations_only("歓楽", "かんらく"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("有り得べき", "ありうべき"),
            self.yd.lookup_translations_only("有り得る", "ありうる"),
        )

        self.assertCountEqual(
            self.yd.lookup_translations_only("宇", "のき"),
            ["свисающий край крыши, стреха", "《перен.》 крыша над головой, " "дом"],
        )

        self.assertCountEqual(
            self.yd.lookup_translations_only("総角", "あげまき"),
            [
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("総角", "あげまき"),
            self.yd.lookup_translations_only("揚巻", "あげまき"),
        )

        # multiple meanings
        self.assertIn(
            self.yd.lookup_translations_only("アイブローペンシル", "あいぶろうぺんしる")[0],
            self.yd.lookup_translations_only("アイブロー", "あいぶろう"),
        )
        self.assertIn(
            self.yd.lookup_translations_only("眉", "まゆ")[0],
            self.yd.lookup_translations_only("アイブロー", "あいぶろう"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("鵺的", "ぬえてき"),
            ["〈~na〉 таинственный, загадочный"],
        )

        # literary variants are marked as such
        self.assertIn(
            self.yd.lookup_translations_only("飽きる", "あきる")[0],
            self.yd.lookup_translations_only("飽く", "あく")[0],
        )

        # obsolete variants are marked as such
        self.assertIn(
            self.yd.lookup_translations_only("侮る", "あなどる")[0],
            self.yd.lookup_translations_only("侮る", "あなずる")[0],
        )

        # different suggestions for different on'yomi
        self.assertCountEqual(
            self.yd.lookup_translations_only("音", "おん"),
            [
                "〈в сочет.〉 звук [как единица речи]",
                "〈в сочет.〉 он (китайское чтение иероглифа)",
                "〈в сочет.〉 звук",
                "звук",
                "〈в сочет.〉 весть",
                "〈в сочет.〉 музыкальный звук",
                "«он» (японизированное китайское чтение иероглифа)",
            ],
        )
        self.assertCountEqual(
            self.yd.lookup_translations_only("音", "いん"),
            [
                "〈в сочет.〉 звук [как единица речи]",
                "〈в сочет.〉 он (китайское чтение иероглифа)",
                "〈в сочет.〉 весть",
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("紙垂", "しで"),
            ["《синт.》 зигзагообразная бумажная лента (исп. в различных " "ритуалах)"],
        )

        self.assertEqual(
            self.yd.lookup_translations_only("春眠暁を覚えず"),
            ["《посл.》 весной спишь так, как будто рассвет никогда не наступит"],
        )
        self.assertCountEqual(
            self.yd.lookup_translations_only("暁", "あかつき")
            + self.yd.lookup_translations_only("暁", "ぎょう"),
            self.yd.lookup_translations_only("暁"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("椦"), ["《бот.》 сумах (от «nurude»)"]
        )
    ...
    def test_general(self):
            ...
            len(
                [
                    ...
                    for item, count in collections.Counter(
                        [e.eid for e in self.yd._entries]
                    ).items()
                    if count > 1
                ]
            ),
        ...
        for e in [
            e for e in self.yd._entries if not e.references and not e.translation
        ]:
            print(e)
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("感無量", "かんむりょう"),
            [
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("寒夜", "かんや"),
            ["《кн.》 холодная (зимняя) ночь"],
        )
        self.assertEqual(
            self.yd.lookup_translations_only("紡ぎ", "つむぎ"),
            ["шелковая ткань типа чесучи"],
        )
        self.assertCountEqual(
            self.yd.lookup_translations_only("阿", "あ"),
            [
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("紡ぎ歌", "つむぎうた"),
            ["песня, исполняемая в процессе прядения"],
        )
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("淫りがわしい", "みだりがわしい"),
            self.yd.lookup_translations_only("淫ら", "みだら"),
        )

        # general translations with complicated markdown
        self.assertEqual(
            self.yd.lookup_translations_only("加圧", "かあつ"),
            [
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("結構", "けっこう"),
            [
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("重畳", "ちょうじょう"),
            ["〈~de aru〉 громоздиться; 〈~desu!〉 превосходно!, прекрасно!"],
        )

        self.assertEqual(
            self.yd.lookup_translations_only("どうやって", "どうやって"), ["как?, каким образом?"]
        )

        self.assertEqual(
            self.yd.lookup_translations_only("何方", "どなた"), ["《вежл.》 кто?"]
        )
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("出だす", "いだす"),
            [
                "《уст.》 помещать (в печати)",
                "《уст.》 посылать, отправлять",
                "《уст.》 выставлять; высовывать; вынимать; вытаскивать",
                "《уст.》 давать, выдавать",
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("赤裸", "あかはだか"),
            ["〈~no〉 совершенно голый; 〈~de〉 голым", "〈~no〉 абсолютный голый"],
        )

        self.assertEqual(
            self.yd.lookup_translations_only("葫蘆", "ころ"), ["《редк.》 тыква-горлянка"]
        )

        self.assertCountEqual(
            self.yd.lookup_translations_only("アウトコース"),
            [
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("一寸", "ちょっと"),
            [
                "〈~shita〉 маленький, слабый, пустяковый",
                "〈~shita〉 приличный, порядочный, неплохой",
                "《с отриц.》 никак, нелегко, вряд ли",
                "немножко, чуточку; на минутку",
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("石", "こく"),
            [
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("よう", "よう"),
            [
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("どっこい"),
            [
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("まあまあ"),
            [
                "〈[~no]〉 неплохой, годный, удовлетворительный, более-менее; 〈~da〉 нормально, сойдет, ничего",
                "полноте, довольно, ладно уже",
                "《межд.》 ох!, ах!",
            ],
        )

        self.assertCountEqual(
            self.yd.lookup_translations_only("ヤード", "やあど"),
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("諄", "じゅん"),
            ["〈в сочет.〉 назойливый", "《мужское имя》"],
        )

        self.assertEqual(
            self.yd.lookup_translations_only("吾", "ご"),
            self.yd.lookup_translations_only("吾", "あが"),
        )

        self.assertEqual(
            self.yd.lookup_translations_only("垢", "こう"), ["〈в сочет.〉 грязь"]
        )

        self.assertEqual(
            self.yd.lookup_translations_only("明け六つ", "あけむつ"),
            ["《уст.》 шесть часов утра"],
        )
    ...
    def test_double_h(self):
        self.assertEqual(
            self.yd.lookup_translations_only("暁には", "あかつきには"), ["когда; в случае"]
        )
        self.assertEqual(
    ...
    def test_no_empty_translations(self):
        ...
        # 合せる -> 合わせる
        self.assertCountEqual(
            self.yd.lookup_translations_only("勠せる", "あわせる"),
            [
                "[при] соединять",
                "согласовывать",
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("裏", "うち"),
            ["〈-no〉＿〈~ni〉 в (условиях чего-л.)"],
        )
        self.assertEqual(
            self.yd.lookup_translations_only("裡", "うち"),
            self.yd.lookup_translations_only("裏", "うち"),
        )

        # 逆う -> 逆らう
        self.assertEqual(
            self.yd.lookup_translations_only("忤う", "さからう"), ["идти против 《чего-л.》"]
        )

        # 華しい -> 華々しい
        self.assertEqual(
            self.yd.lookup_translations_only("花々しい", "はなばなしい"),
            ["〈~na〉 яркий, цветущий, прекрасный"],
        )

        # 捻くれる -> 捻れる
        self.assertEqual(
            self.yd.lookup_translations_only("拈くれる", "ひねくれる"),
            ["быть искривленным; быть замысловатым; " "быть извращенным"],
        )

        # 芽む -> 芽ぐむ
        self.assertEqual(
            self.yd.lookup_translations_only("萌む", "めぐむ"),
            ["давать почки; распускаться"],
        )

        # 別かれ -> 別れ
        self.assertCountEqual(
            self.yd.lookup_translations_only("分かれ", "わかれ"),
        ...
        # ちじみあがる -> ちぢみあがる
        self.assertCountEqual(
            self.yd.lookup_translations_only("縮み上がる", "ちぢみあがる"),
            ["сжиматься; съеживаться"],
        )

        # いんなーわあ -> いんなーうぇあ
        self.assertCountEqual(
            self.yd.lookup_translations_only(lexeme="インナーウェア", reading="いんなあうぇあ"),
            ["нательное (нижнее) белье (англ. 《innerware》)", "нижнее белье"],
        )

        # дюйм\(\англ.#\inchi\#) -> дюйм\(\англ.#\inch\#)
        self.assertEqual(
            self.yd.lookup_translations_only("吋", "いんち"), ["дюйм (англ. 《inchi》)"]
        )

        # <<кино>> ср -> <<кино>> см
        self.assertCountEqual(
            self.yd.lookup_translations_only("アップ"),
            [
                ...
                "《тех.》 кривошип (англ. 《crank》)",
                "рукоятка (особ. кинокамеры старого образца)",
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("輪形", "わがた"),
            ["кольцо, круг; 〈~no〉 кольцевидный, круглый"],
        )

        # とうる -> とおる
        self.assertEqual(
            self.yd.lookup_translations_only("透る", "とおる"),
            ["проникать, проходить сквозь"],
        )
        self.assertCountEqual(
            self.yd.lookup_translations_only("亮", "とおる"),
            ["《мужское имя》", "〈в сочет.〉 ясный, очевидный"],
        )

    def test_matching_lexemes(self):
        self.assertEqual(
            self.yd.lookup_translations_only("繰り返す", "くりかえす"),
            self.yd.lookup_translations_only("繰返す", "くりかえす"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("相打ち", "あいうち"),
            self.yd.lookup_translations_only("相打", "あいうち"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("空地", "あきち"),
            self.yd.lookup_translations_only("空き地", "あきち"),
        )
        self.assertEqual(self.yd.lookup_translations_only("お前", "おまえ"), ["《грубо》 ты"])
        self.assertCountEqual(
            self.yd.lookup_translations_only("前", "まえ"),
            [
                "〈в сочет.〉 порция; доля",
                "〈~ni〉 заранее",
                "〈в сочет.〉 предшествующий",
                "гениталии",
                "〈в сочет.〉 заранее",
                "перед; 〈~ni〉 впереди; 〈~no〉 передний",
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("前", "ぜん"),
            [
                "〈в сочет.〉 порция; доля",
                "〈в сочет.〉 предшествующий",
                "〈в сочет.〉 заранее",
    ...
    def test_search_modes(self):
        ...
        self.assertEqual(
            self.yd.lookup_translations_only("仕舞う", "しまう"),
            self.yd.lookup_translations_only("しまう", "しまう"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("終う", "しまう"),
            self.yd.lookup_translations_only("了う", "しまう"),
        )
        self.assertEqual(
            self.yd.lookup_translations_only("了う", "しまう"),
            self.yd.lookup_translations_only("しまう", "しまう"),
        ...
        self.assertEqual(
            self.yd.lookup_translations_only(
                "逆う", "さからう", search_mode=SearchMode.shallow_only
            ),
            [],
        )
        self.assertEqual(
            self.yd.lookup_translations_only(
                "逆う", "さからう", search_mode=SearchMode.deep_only
            ),
            ["идти против 《чего-л.》"],
        )
        self.assertEqual(
            self.yd.lookup_translations_only(
                "逆う", "さからう", search_mode=SearchMode.consecutive
            ),
            ["идти против 《чего-л.》"],
        )
    ...
    def test_orders(self):
        ...
        self.assertCountEqual(
            self.yd.lookup_translations_only("繰返す", order=1),
            ["повторять, делать снова"],
        )

        # order = 2: the candidates with the two topmost probability scores
        self.assertCountEqual(
            self.yd.lookup_translations_only("繰返す", order=2),
